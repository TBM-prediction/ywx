...
class Config:
    ...
    def __init__(self, debug=True, gpu_start=2, sl=30, selected_columns=False, is_problem1=True, valid_ratio=0.2,
            postpond=0, font='/usr/share/fonts/wqy-microhei/wqy-microhei.ttc'):
        # font='/System/Library/Fonts/PingFang.ttc'
        self.debug,self.gpu_start,self.sl,self.selected_columns,self.postpond,font = \
                debug,gpu_start,sl,selected_columns,postpond,font
        self.mulr = mulr = 7
        if selected_columns:
            # self.cont_names = ['推进速度', '主驱动1#电机扭矩', '刀盘功率', '刀盘扭矩', '刀盘转速','主液压油箱温度', '前点偏差X', '总推进力']
            self.cont_names = ['桩号', '刀盘功率', '主驱动1#电机扭矩', '主驱动1#电机输出功率', '推进速度', '总推进力',
            '刀盘扭矩', '贯入度', '推进位移']
            self.n_cont = len(self.cont_names)
        else:
            self.n_cont = 5 if debug else 192
        num_cycles = self.num_cycles = 300 if debug else 3481
